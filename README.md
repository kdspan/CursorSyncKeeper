...
核心目标：**游戏运行时也能正常使用副屏**——这是本工具区别于单纯「改注册表」方案的关键。

## 问题根因

Windows 默认使用 **硬件鼠标光标**（GPU 的 Multiplane Overlay，MPO 平面）绘制指针。硬件光标由显示控制器直接合成，不进帧缓冲，于是：

- 某些全屏应用 / 显示驱动重置会把系统切回硬件鼠标路径，并且之后**不会自动恢复**；
- 硬件光标平面无法跨多个显示器一致合成，在「游戏主屏 + 副屏」的双屏场景下极易出现副屏光标丢失或卡死；
- 软件鼠标（由 DWM 合成进主平面）才是跨屏稳定可用的方案，但系统并不会自动回到它。
