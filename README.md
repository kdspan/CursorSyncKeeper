...
- **根治原理**：从 `HKCU\Control Panel\Mouse\MouseTrails` 读取用户已保存的值，
  但**不是简单地把同一个值写回**——那样系统判定“无变化”而不会重新应用，
  视觉状态依然丢失。正确做法是**强制一次状态切换**：先切到相反状态（触发内核
  重建鼠标拖影渲染管线），再写回真实值并 `SPIF_UPDATEINIFILE|SPIF_SENDCHANGE`
  持久化+广播，最后用 `SPI_SETCURSORS` 让系统重新加载整套光标方案，
  端到端地“重新读取并应用”设置，覆盖驱动重置导致的“内核状态丢失”。
